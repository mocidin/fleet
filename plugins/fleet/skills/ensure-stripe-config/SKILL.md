...
5. **Price catalog + payment links — only what is on sale exists.**
   ...
   - **Recruiterbase has no payment links since 2026-09-09**: checkout is minted server-side (`app/api/stripe/checkout/route.ts`) from a list price ($78 IB, $158 ALL) plus the one pre-applied `forever` coupon `NETWORKING_SEASON` (Networking Season Discount, 50% off, so checkout lands on $39 and $79), because a payment link cannot pre-apply a discount. Its price change is then: create the new price → repoint `default_price` → update the ids in that route → deploy → archive the old price. Coupons are catalog too: one coupon, named for the reason, nothing left behind that no checkout applies.
