...
1. **Customer portal — ONE all-features preset (the important one).**
   ...
   - That default has **all three features enabled**, with these exact behaviors (verified identical on all four accounts 2026-08-10):
     - `subscription_update`: `default_allowed_updates` includes `price`, `proration_behavior = always_invoice`, `schedule_at_period_end.conditions = [decreasing_item_amount]`. Upgrades bill immediately and prorate; downgrades wait for period end, so nobody buys a cheaper month mid-cycle.
       ...
       - **`enabled` only when 2 or more such prices exist.** On a single-tier account Stripe refuses the flow outright (`no price in the portal configuration available to change to`), so enabling it can only produce a dead end. Brandflare and StonedGPT are single-tier and correctly have it **disabled**; flip it on the moment a second tier ships. Verify the app's UI matches: Brandflare drives cancel/reactivate/payment_method flows plus a payment link, StonedGPT opens the portal home with no `flow_data`, so neither surfaces a plan-change control.
   ...
   - Reference implementation: **Ghostplug** `app/api/stripe/billing/route.ts` (Brandflare's is the same file) — one default preset, `flow_data` to steer, four flows (`update`/`cancel`/`reactivate`/`payment_method`), and `return_url` validated as a same-origin relative path against open redirects. Recruiterbase and StonedGPT expose only the `subscription_update` flow and should be brought up to the four-flow route when next touched.
...
5. **Price catalog + payment links — only what is on sale exists.**
   ...
   - A price is **in use** only if one of these is true: an **active payment link** sells it, a subscription in `active/trialing/past_due/unpaid/paused` is billed on it, or it is the `default_price` of a product still on sale. Everything else gets archived. Never judge by amount or name.
   ...
   - Old payment links get `active=false` once the app's env vars point at the new ones. Links cannot be deleted, only deactivated, and a stale link in an ad or bio will read "no longer active", so the price rise is worthless until the app's own env vars are updated.
   - **Price-change runbook** (the whole sequence, in order): create the new price → repoint the product's `default_price` → archive the old price → create a new payment link → update `NEXT_PUBLIC_STRIPE_*_LINK` on **Vercel Production** → redeploy (these are `NEXT_PUBLIC_`, so they bake at build time) → watch the deploy to Ready → deactivate the old link → rewrite the portal config's products list.
   - **Recruiterbase has no payment links since 2026-09-09**: checkout is minted server-side (`app/api/stripe/checkout/route.ts`) from the real monthly price ($39 IB `price_1U35MT…`, $79 ALL `price_1U35MU…`) plus a first-month coupon (`FIRST_MONTH_IB` $20 off, `FIRST_MONTH_ALL` $40 off, duration `once`) applied only while that user's 48 hour offer (`lib/offer.ts`) is still running, because a payment link cannot apply a discount conditionally. Both products' `default_price` and the portal's plan-change list are those same two prices (set 2026-09-17, when the earlier list-price scheme, $78/$158 plus a 50% forever coupon, was archived). Its price change is then: create the new price → repoint `default_price` → update the id in that route → deploy → archive the old price → rewrite the portal list. Coupons are catalog too: only the coupons a checkout applies exist, named for the reason.
