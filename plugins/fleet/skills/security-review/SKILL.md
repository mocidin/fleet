...
- **Dependency advisories**: a direct runtime dependency counts when the advisory is an auth bypass the app relies on (a Next.js proxy/middleware bypass on a version the app runs, checked against the advisory's own preconditions such as i18n or Pages Router) or a critical whose vulnerable feature this deployment actually executes. On Vercel, `/_next/image` is served by the platform's own optimizer, so image-optimizer advisories (the AVIF RCE class) do not apply to fleet apps; Windows-only advisories never do; DoS-only advisories are below the bar. One row per package, severity high for a shell-only proxy bypass, remedy = the fixed version via the stack update.
