---
name: optimize-seo
description: Audit and upgrade SEO for the whole Hypertheory fleet surface — the product apps (Ghostplug, Brandflare, StonedGPT, Recruiterbase, Hypertheory, and any app added to the fleet later; the skill discovers the current roster at run time) to current Next.js App Router best practices, AND the cold-outreach sales domains (email-landing project) whose job is staying OUT of the index — verified against live Google Search Console data plus live SERP checks (Serper). Self-evolving — every run re-verifies best practices via web research against official sources and updates this skill's own evolving sections. Covers metadata, robots.ts, sitemap.ts, canonicals, JSON-LD structured data, OG images, SEO content pages, auth-proxy interception of SEO routes, and sales-domain deindexing — using Recruiterbase as the in-fleet reference implementation. Use when the user asks to "optimize SEO", "audit SEO", "why isn't X ranking", "why is a sales domain ranking", "add a sitemap/robots", "add SEO pages / blog posts", or "compare SEO across apps".
allowed-tools:
  - Agent
  - Glob
  - Grep
